              ...
              >
                {slides.map((slide, index) => (
                  <div className="slide-item" key={index}>
                    <div style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.15em', opacity: 0.6, marginBottom: '24px' }}>
                      {slide.tagline}
                    </div>
                    <div className="slide-icon" style={{ marginBottom: '24px', opacity: 0.9 }}>{slide.icon}</div>
            ...
            <div className="carousel-dots" style={{ marginTop: '60px' }}>
              {slides.map((_, index) => (
                <div 
                  ...
                  style={{ background: activeSlide === index ? 'var(--white)' : 'rgba(255, 255, 255, 0.3)' }}
                />
              ))}
            </div>
          </div>
          ...
          <div className="auth-form-wrapper">
            ...
            {(title || subtitle) && (
              <div className="auth-header" style={{ marginBottom: '32px' }}>
                {title && <h1 style={{ fontSize: '32px', fontWeight: 800, marginBottom: '8px' }}>{title}</h1>}
